...
document.getElementById('btnSi').addEventListener('click', async () => {
  ...
  try {
    if (pendiente.accion === 'estado') {
      // 1. Determina si actualmente está activo antes de hacer cualquier cambio
      const estaActivo = u.estado === 'activo' || u.estado === 1;
      const nuevoEstadoActivo = !estaActivo; // Invierte el estado actual

      // 2. Se envía la petición con el valor invertido correcto
      await api(`/api/credenciales/${pendiente.id}/estado`, {
        method: 'PATCH',
        body: JSON.stringify({ activo: nuevoEstadoActivo })
      });

      avisar(`${u.nombre} quedó ${nuevoEstadoActivo ? 'activo' : 'inactivo'}.`);

      // 3. Recarga los usuarios directamente de la base de datos para refrescar la tabla
    ...
    avisar('Ocurrió un error al intentar cambiar el estado.');
  } finally {
    pendiente = null;
...
async function cambiarEstadoUsuario(id, nuevoEstadoActivo) {
  try {
    await api(`/api/credenciales/${id}/estado`, {
      method: 'PATCH',
      body: JSON.stringify({ activo: nuevoEstadoActivo })
    });
    
    await cargarUsuarios(); // Refresca la tabla con los datos actualizados de SQL
  } catch (error) {
    console.error('No se pudo cambiar el estado del usuario:', error);
    avisar('Error al actualizar el estado del usuario.');
  }
}

// Función que se ejecuta desde el menú de acciones de la fila (botón Activar/Desactivar)
function pedirCambiarEstado(id) {
  const u = usuarios.find(x => x.id === id);
  if (!u) return;

  // Asigna los datos necesarios a la variable global 'pendiente'
  pendiente = { 
    id: id, 
    accion: 'estado',
    nuevoEstadoActivo: u.estado !== 'activo' // Si está inactivo, pasará a true (activo)
  };
}
